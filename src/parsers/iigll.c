...
init(nmea_parser_s *parser)
{
	...
	NMEA_PARSER_TYPE(parser, NMEA_IIGLL);
	NMEA_PARSER_PREFIX(parser, "IIGLL");
	return 0;
}
...
{
	parser->data = malloc(sizeof (nmea_iigll_s));
	if (NULL == parser->data) {
		return -1;
	}
...
{
	memset(parser->data, 0, sizeof (nmea_iigll_s));
	return 0;
}
...
{
	nmea_iigll_s *data = (nmea_iigll_s *) parser->data;

	switch (val_index) {
	case NMEA_IIGLL_TIME:
		/* Parse time */
	...
	case NMEA_IIGLL_LATITUDE_CARDINAL:
	...
	case NMEA_IIGLL_LONGITUDE_CARDINAL:
		/* Parse cardinal direction */
